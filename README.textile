...
h3. >Extending - Event Hooks
* <code>slideshow_start</code> - slideshow started
* <code>slideshow_stop</code> - slideshow stopped
* <code>slideshow_paused</code> - slideshow paused
* <code>slideshow_unpaused</code> - slideshow unpaused - may not trigger properly or match a pause event if user clicks play/stop button
* <code>slide_init</code> - triggered when panel slide is initialized (before controls animate)
* <code>slide_begin</code> - triggered before panel slide animation
* <code>slide_complete</code> - triggered after panel slide animation

You can bind to any of the custom event triggers as follows (see this page source for another example):<br>

<pre>$('#slider1').bind('slide_complete', function(e, el){
 $('#status').text( 'Now on page #' + $(el).data('AnythingSlider').currentPage );
 // Do something else
});</pre>

<pre>$('.anythingSlider').bind('slideshow_start slideshow_stop', function(e, el){
